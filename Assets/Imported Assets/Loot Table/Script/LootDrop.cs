...
namespace TinyScript {
    ...
    public class LootDrop : ScriptableObject
    {
        ...
        public List<Item> GetGuaranteeedLoot()
        {
        ...
        public List<Item> SpawnDrop(int _count)
        {
            List<Item> guaranteed = GetGuaranteeedLoot();
            List<Item> randomLoot = GetRandomLoot(_count);
            List<Item> lootList = new List<Item>();

            for (int i = 0; i < guaranteed.Count; i++)
            {
                lootList.Add(guaranteed[i]);
            }

            for (int i = 0; i < randomLoot.Count; i++)
            {
                lootList.Add(randomLoot[i]);
            }
        ...
        /* Fixed by D9Construct */
            ...
            if (EditorGUI.EndChangeCheck())
            {
                for (int index = 0; index < loot.OneItemFromList.Length; index++)
                {
                    ...
                    loot.OneItemFromList[index].Drop = (Item)OIFElement.FindPropertyRelative("Drop").objectReferenceValue;
                ...
                for (int index = 0; index < loot.GuaranteedLootTable.Length; index++)
                {
                    ...
                    loot.GuaranteedLootTable[index].Drop = (Item)GuaranteedElement.FindPropertyRelative("Drop").objectReferenceValue;
